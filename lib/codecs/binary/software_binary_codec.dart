...
final class SoftwareBinaryCodec implements SoftwareCodec {
  ...
  static final _file = File('lib/out.bin');

  @override
  void write(List<Software> software) {
    final bytes = _codec.encodeMessage(software.map((e) => e.toObjects()).toList()) ?? ByteData(0);
    _file.writeAsBytesSync(bytes.buffer.asUint8List(bytes.offsetInBytes, bytes.lengthInBytes));
  }
  ...
  List<Software> read() {
    ...
    return objects.map((e) => Software.fromObjects(e as List<Object?>)).toList();
  }
}
